...
app.use('/api', async (req, res, next) => {
  try {
    // 跳過不需要密碼的端點
    if (req.path === '/health' || req.path === '/verify-password' || req.path === '/debug-lotsize' || req.path === '/migrate' || req.path.startsWith('/lotsize/') || (req.path === '/trigger-lotsize-update' && req.method === 'GET')) {
      await initStorage();
      return next();
    }
    // 密碼檢查
